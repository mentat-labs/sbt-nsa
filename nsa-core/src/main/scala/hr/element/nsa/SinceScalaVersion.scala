...
object SinceScalaVersion {
  def apply(epoch: Int, major: Int): SinceScalaVersion =
    SinceScalaVersion(epoch, major, 0, None)
  ...
  private val ScalaVersionPattern = """(\d+?)\.(\d+?)\.(\d+?)(?:-(\d+?))?"""r
  def apply(scalaVersion: String): Option[SinceScalaVersion] = Try {
    val ScalaVersionPattern(epochStr, majorStr, minorStr, patchStr) = scalaVersion
    ...
    SinceScalaVersion(epoch, major, minor, patch)
  }.toOption
}
