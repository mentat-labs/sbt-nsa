...
class ScalacOptionsDSLPrivateSpec extends ScalacOptionsDSLSpec {
  def is = s2"""
...
"""
  ...
  def `-Ybackend test` = new ScalacOptions { def options = Seq(
    -Ybackend GenASM    , "-Ybackend:GenASM"
  , -Ybackend GenBCode  , "-Ybackend:GenBCode"
  , -Ybackend ("FooBar"), "-Ybackend:FooBar"
  )}

  def `-Ybreak-cycles test` = new ScalacOptions { def options = Seq(
    -Ybreak-cycles, "-Ybreak-cycles"
  )}

  def `-Yclosure-elim test` = new ScalacOptions { def options = Seq(
    -Yclosure-elim, "-Yclosure-elim"
  )}

  def `-Ycompact-trees test` = new ScalacOptions { def options = Seq(
    -Ycompact-trees, "-Ycompact-trees"
  )}

  def `-Yconst-opt test` = new ScalacOptions { def options = Seq(
    -Yconst-opt, "-Yconst-opt"
  )}

  def `-Ydead-code test` = new ScalacOptions { def options = Seq(
    -Ydead-code, "-Ydead-code"
  )}

  def `-Ydebug test` = new ScalacOptions { def options = Seq(
    -Ydebug, "-Ydebug"
  )}

  def `-Ydelambdafy test` = new ScalacOptions { def options = Seq(
    -Ydelambdafy inline    , "-Ydelambdafy:inline"
  , -Ydelambdafy method    , "-Ydelambdafy:method"
  , -Ydelambdafy ("FooBar"), "-Ydelambdafy:FooBar"
  )}

  def `-Ydisable-unreachable-prevention test` = new ScalacOptions { def options = Seq(
    -Ydisable-unreachable-prevention, "-Ydisable-unreachable-prevention"
  )}

  def `-Yinline test` = new ScalacOptions { def options = Seq(
    -Yinline, "-Yinline"
  )}

  def `-Yinline-handlers test` = new ScalacOptions { def options = Seq(
    -Yinline-handlers, "-Yinline-handlers"
  )}

  def `-Yinline-warnings test` = new ScalacOptions { def options = Seq(
  ...
  def `-Ylog-classpath test` = new ScalacOptions { def options = Seq(
    -Ylog-classpath, "-Ylog-classpath"
  )}

  def `-Ymacro-debug-lite test` = new ScalacOptions { def options = Seq(
    -Ymacro-debug-lite, "-Ymacro-debug-lite"
  )}

  def `-Ymacro-debug-verbose test` = new ScalacOptions { def options = Seq(
    -Ymacro-debug-verbose, "-Ymacro-debug-verbose"
  )}

  def `-Ymacro-expand test` = new ScalacOptions { def options = Seq(
  ...
  def `-Yno-adapted-args test` = new ScalacOptions { def options = Seq(
    -Yno-adapted-args, "-Yno-adapted-args"
  )}

  def `-Yno-completion test` = new ScalacOptions { def options = Seq(
    -Yno-completion, "-Yno-completion"
  )}

  def `-Yno-generic-signatures test` = new ScalacOptions { def options = Seq(
    -Yno-generic-signatures, "-Yno-generic-signatures"
  )}

  def `-Yno-imports test` = new ScalacOptions { def options = Seq(
    -Yno-imports, "-Yno-imports"
  )}

  def `-Yno-load-impl-class test` = new ScalacOptions { def options = Seq(
    -Yno-load-impl-`class`, "-Yno-load-impl-class"
  , -Yno-load-impl_class  , "-Yno-load-impl-class"
  )}

  def `-Yno-predef test` = new ScalacOptions { def options = Seq(
    -Yno-predef, "-Yno-predef"
  )}

  def `-Ynooptimise test` = new ScalacOptions { def options = Seq(
    -Ynooptimise, "-Ynooptimise"
  )}

  def `-Yopt test` = new ScalacOptions { def options = Seq(
    -Yopt `_`                ,             "-Yopt:_"
    , -Yopt `help`, "-Yopt:help"
  ...
  def `-Yoverride-objects test` = new ScalacOptions { def options = Seq(
    -Yoverride-objects, "-Yoverride-objects"
  )}

  def `-Yoverride-vars test` = new ScalacOptions { def options = Seq(
    -Yoverride-vars, "-Yoverride-vars"
  )}

  def `-Ypresentation-strict test` = new ScalacOptions { def options = Seq(
    -Ypresentation-strict, "-Ypresentation-strict"
  )}

  def `-Ypresentation-verbose test` = new ScalacOptions { def options = Seq(
    -Ypresentation-verbose, "-Ypresentation-verbose"
  )}

  def `-Yrangepos test` = new ScalacOptions { def options = Seq(
    -Yrangepos, "-Yrangepos"
  )}

  def `-Yreify-copypaste test` = new ScalacOptions { def options = Seq(
    -Yreify-copypaste, "-Yreify-copypaste"
  )}

  def `-Yrepl-class-based test` = new ScalacOptions { def options = Seq(
    -Yrepl-`class`-based, "-Yrepl-class-based"
  , -Yrepl_class-based  , "-Yrepl-class-based"
  , -Yrepl-class_based  , "-Yrepl-class-based"
  )}

  def `-Yrepl-sync test` = new ScalacOptions { def options = Seq(
    -Yrepl-sync, "-Yrepl-sync"
  )}

  def `-Yresolve-term-conflict test` = new ScalacOptions { def options = Seq(
  ...
  def `-Yshow-symkinds test` = new ScalacOptions { def options = Seq(
    -Yshow-symkinds, "-Yshow-symkinds"
  )}

  def `-Yshow-symowners test` = new ScalacOptions { def options = Seq(
    -Yshow-symowners, "-Yshow-symowners"
  )}

  def `-Yshow-syms test` = new ScalacOptions { def options = Seq(
    -Yshow-syms, "-Yshow-syms"
  )}

  def `-Yshow-trees test` = new ScalacOptions { def options = Seq(
    -Yshow-trees, "-Yshow-trees"
  )}

  def `-Yshow-trees-compact test` = new ScalacOptions { def options = Seq(
    -Yshow-trees-compact, "-Yshow-trees-compact"
  )}

  def `-Yshow-trees-stringified test` = new ScalacOptions { def options = Seq(
    -Yshow-trees-stringified, "-Yshow-trees-stringified"
  )}

  def `-Yskip test` = new ScalacOptions { def options = Seq(
  ...
  def `-Ystatistics test` = new ScalacOptions { def options = Seq(
  ...
  def `-Ystop-after test` = new ScalacOptions { def options = Seq(
  ...
  def `-Ystop-before test` = new ScalacOptions { def options = Seq(
  ...
  def `-Ywarn-adapted-args test` = new ScalacOptions { def options = Seq(
    -Ywarn-adapted-args, "-Ywarn-adapted-args"
  )}

  def `-Ywarn-dead-code test` = new ScalacOptions { def options = Seq(
    -Ywarn-dead-code, "-Ywarn-dead-code"
  )}

  def `-Ywarn-inaccessible test` = new ScalacOptions { def options = Seq(
    -Ywarn-inaccessible, "-Ywarn-inaccessible"
  )}

  def `-Ywarn-infer-any test` = new ScalacOptions { def options = Seq(
    -Ywarn-infer-any, "-Ywarn-infer-any"
  )}

  def `-Ywarn-nullary-override test` = new ScalacOptions { def options = Seq(
    -Ywarn-nullary-`override`, "-Ywarn-nullary-override"
  , -Ywarn-nullary_override  , "-Ywarn-nullary-override"
  )}

  def `-Ywarn-nullary-unit test` = new ScalacOptions { def options = Seq(
    -Ywarn-nullary-unit, "-Ywarn-nullary-unit"
  )}

  def `-Ywarn-numeric-widen test` = new ScalacOptions { def options = Seq(
    -Ywarn-numeric-widen, "-Ywarn-numeric-widen"
  )}

  def `-Ywarn-unused test` = new ScalacOptions { def options = Seq(
    -Ywarn-unused, "-Ywarn-unused"
  )}

  def `-Ywarn-unused-import test` = new ScalacOptions { def options = Seq(
    -Ywarn-unused-`import`, "-Ywarn-unused-import"
  , -Ywarn-unused_import  , "-Ywarn-unused-import"
  )}

  def `-Ywarn-value-discard test` = new ScalacOptions { def options = Seq(
