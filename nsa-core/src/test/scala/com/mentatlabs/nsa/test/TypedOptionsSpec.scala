...
trait TypedOptionsSpec extends Specification {
  protected implicit class PimpedTypedOption(typedOption: TypedOption) {
    def =>=(expected: String) = {
  ...
  protected implicit def checkVersions(typedOption: TypedOption): SpecPart = {
    ...
    val pattern = " " + (typedOption match {
      case _: JustOption => head + "  "
      case _: ChoiceOption[_] => head.replaceFirst(":.*", "(  |:<)")
      case _: ValueOption[_] => head + "(  | <)"
    }) r
